 ...
 * @param {String} view
 */
function navigateTopLevel(nav, view) {
  const navItem = document.querySelector("#nav");
  const navChildren = navItem.children;
  for (let index = 0; index < navChildren.length; index++) {
    if (navChildren.item(index).classList.contains("activated")) {
      navChildren.item(index).classList.remove("activated");
    }

    if (navChildren.item(index).id === nav) {
      navChildren.item(index).classList.add("activated");
    }
  }
...
function populateProductFields() {
  const productDiv = document.getElementById("productView");
